...
class Settings(BaseSettings):  # pylint: disable=too-few-public-methods
    ...
    # aws_accounts: dict[str, list[str]]

    class Config:  # pylint: disable=too-few-public-methods
        """Config class to be used for Settings."""

        env_prefix = "FILE_MUNGING_"
        env_file = ENV_FILE_PATH.resolve()
        env_file_encoding = "utf-8"


def load(config_file_name="pyproject.toml", config_data=None) -> Settings:
    """Loads passed dynamic settings data using `config data` or from a TOML configuration file (pyproject.toml).

    When loaded from file, the settings for this app are expected to be in [tool.file-munging] in TOML
    ...
    # Or load from configuration file
        ...
        if config_file.exists():
            ...
            if "tool" in config_tmp and "file-munging" in config_tmp.get("tool", {}):
                SETTINGS = Settings(**config_tmp["tool"]["file-munging"])
...
def load_or_exit(config_file_name="pyproject.toml", config_data=None) -> Settings:
    """Loads passed dynamic settings data using `config data` or from a TOML configuration file (pyproject.toml).
    ...
    """
    try:
        return load(config_file_name=config_file_name, config_data=config_data)
